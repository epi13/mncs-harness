...
- `agent._exact_manual_route` — **MIGRATION-DEBT** — Exact-pin failure path could execute admit_placement; not yet wired.
- `agent.per-step-decisions` — **MNCS-EXECUTED** — Per-step route/policy/verdict choices execute kernels through tools/router.
- `agent.tool-loop/runtime` — **BLOCKED:HARNESS-PRESSURE-007** — Multi-turn loop, spawning, cancellation need concurrency.
- `atlas_binding._fold_capability` — **MNCS-EXECUTED** — Duplicate-decision fold executes fold(); reasons/outstanding host-side.
- `atlas_binding.confirm_execution` — **MNCS-EXECUTED** — Final target comparison executes dispatch_gate; freshness/signature host-side.
...
- `capability_graph.build_capability_graph` — **MIGRATION-DEBT** — Set-cover over string capability graphs; bounded-graph kernel candidate, not yet attempted.
...
- `commons_operator.publication-policy` — **MIGRATION-DEBT** — Termination-mismatch/publication refusal rules are boolean gates over carried verdicts; convertible, not yet attempted.
...
- `experiment_readiness._overall` — **MNCS-EXECUTED** — Overall verdict executes fold_readiness; warnings stay host-side.
...
- `fabric_compat.evaluate_experiment_fabric` — **MNCS-EXECUTED** — Classification + dispatch gate execute the fabric kernel; version parsing host-side.
...
- `fabric_inventory_session.placement-admission` — **MNCS-EXECUTED** — Pin admission executes admit_placement; reason prefixes host-side.
...
- `fleet.role_availability` — **MIGRATION-DEBT** — Availability counts over inventory dicts; threshold kernel candidate, not yet attempted.
- `metrics.metrics/events` — **BLOCKED:HARNESS-PRESSURE-005** — Timestamps, durations, and SQLite persistence need clock + storage.
- `mncs.atlas-fold` — **MNCS-EXECUTED** — mncs/harness_atlas.mncs fold_pair/fold4/dispatch_gate/tool_admission executed via mncs_exec.
- `mncs.classify_route` — **MNCS-EXECUTED** — mncs/harness_routing.mncs executed via mncs_exec on every route.
- `mncs.eligibility` — **MNCS-EXECUTED** — mncs/harness_eligibility.mncs capability/resource gates executed via mncs_exec.
...
- `mncs.pins` — **MNCS-EXECUTED** — mncs/harness_pins.mncs shape table + fail-closed admission executed via mncs_exec.
- `mncs.readiness-fold` — **MNCS-EXECUTED** — Generic fold_readiness<N> over concrete fold4/fold8 envelopes executed via mncs_exec.
- `mncs.verdict-fold` — **MNCS-EXECUTED** — mncs/harness_verdict.mncs dominate/combine4/combine8 executed; host chunks envelopes.
- `mncs_exec._transitional` — **MIGRATION-DEBT** — Opt-in fallback oracle only; production path never reaches it. Remove when executor is ubiquitous.
- `mncs_exec.primary_role/command_reason/combine/fold/*` — **MNCS-EXECUTED** — Canonical entrypoints; each one executes its kernel (dual-shape decode documented).
- `mncs_logic.corpus_cases` — **TEST-BUILD-ONLY** — Corpus loader reused by tests.
- `mncs_logic.mirrors` — **MIGRATION-DEBT** — Transitional oracle + test fixture. Delete after the transitional window closes.
...
- `model_selection._eligible` — **MNCS-EXECUTED** — Evidence lookup, claim tables, reason strings; verdict per needed capability executes capability_eligible.
...
- `models.RoutingOverride.__post_init__` — **MNCS-EXECUTED** — Pin shape table executes pin_fields_valid; string content checks host-side.
...
- `policy.CommandPolicy.evaluate` — **MNCS-EXECUTED** — Command ordering executes command_reason; lexing/paths host-side.
...
- `policy.file_write_decision` — **MNCS-EXECUTED** — Write decision executes the policy kernel.
...
- `residency._resource_decision` — **MNCS-EXECUTED** — Integer budget/availability verdict executes resource_gate; facts contract unchanged.
...
- `router._deterministic_route` — **MNCS-EXECUTED** — Primary classification + coder chain execute the routing kernel; availability mapping host-side.
...
- `tools._atlas_gate` — **MNCS-EXECUTED** — Granted-AND-covered verdict executes tool_admission; reason text host-side.
...
- `verifiers.Verifier.verify` — **MNCS-EXECUTED** — Verification acceptance folds through combine; subprocess checks host-side.
